...
let achiraCatalogData = [];
try {
  const pData = require('../../products_data.js');
  if (pData && Array.isArray(pData.ACHIRA_PRODUCTS_DATA)) {
    achiraCatalogData = pData.ACHIRA_PRODUCTS_DATA;
  }
} catch (e) {
  try {
    const pData = require(path.join(__dirname, '..', '..', 'products_data.js'));
    if (pData && Array.isArray(pData.ACHIRA_PRODUCTS_DATA)) {
      achiraCatalogData = pData.ACHIRA_PRODUCTS_DATA;
    }
  } catch(err) {}
...
const initialOtherProducts = [
  ...
  { id: 301, sku: "ACH-JWL-301", name: "Concentric Circular Halo Diamond Studs", category: "Jewellery", fabric: "Gemstones", color: "Gold", size: "Standard", price: null, availability: "New Arrival", occasion: "Festive", image: "jewellery1.jpg", rating: 5 },
  { id: 302, sku: "ACH-JWL-302", name: "Celestial Crescent Diamond Earrings", category: "Jewellery", fabric: "Gemstones", color: "Gold", size: "Standard", price: null, availability: "New Arrival", occasion: "Bridal", image: "jewellery2.jpg", rating: 5 },
  { id: 303, sku: "ACH-JWL-303", name: "Solitaire Diamond Riviera Necklace", category: "Jewellery", fabric: "Gemstones", color: "Gold", size: "Adjustable", price: null, availability: "Best Seller", occasion: "Wedding", image: "jewellery3.jpg", rating: 5 },
  { id: 401, sku: "ACH-RNG-401", name: "Aura Wave Diamond Cluster Ring", category: "Jewellery", fabric: "Gemstones", color: "Gold", size: "Adjustable", price: null, availability: "New Arrival", occasion: "Anniversary", image: "ring1.jpg", rating: 5 },
  { id: 402, sku: "ACH-RNG-402", name: "Empress Oval Halo Floral Diamond Ring", category: "Jewellery", fabric: "Gemstones", color: "Gold", size: "Adjustable", price: null, availability: "Best Seller", occasion: "Daily Wear", image: "ring2.jpg", rating: 5 }
];

const memoryStore = {
  users: [],
  admins: [],
  products: [...achiraCatalogData, ...initialOtherProducts],
  orders: [],
  coupons: [
    ...
    if (fs.existsSync(dbFilePath)) {
      ...
      if (data && typeof data === 'object') {
        ...
        if (Array.isArray(data.products) && data.products.length > 0) {
          ...
          if (achiraCatalogData.length > 0 && !prods.some(p => String(p.id).startsWith('straight-'))) {
            memoryStore.products = [...achiraCatalogData, ...prods];
          } else {
            memoryStore.products = prods;
          }
        } else if (achiraCatalogData.length > 0) {
          memoryStore.products = [...achiraCatalogData, ...initialOtherProducts];
        }
